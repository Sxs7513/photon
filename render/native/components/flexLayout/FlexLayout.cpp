...
FlexLayout::FlexLayout(QWidget* parentWidget, YGNodeRef parentNode) : QLayout(parentWidget) {
    ...
    // Throttle the setGeometry calls that may happen when dealing with huge lists.
    this->throttleTimer.setTimerType(Qt::PreciseTimer);
    this->throttleTimer.setSingleShot(true);
    QObject::connect(&this->throttleTimer, &QTimer::timeout, this, &FlexLayout::performLayout);
}
